...
public class PostagemController {
	...
	@Autowired // Injeção de dependências, implementa as ferramentas do repository nessa classe
	private PostagemRepository postagemRepository;

	@GetMapping
	public ResponseEntity<List<Postagens>> getAll() { // getAll pega tudo da lista
		return ResponseEntity.ok(postagemRepository.findAll());
	}

	@GetMapping("/{id}") // busca por id
	public ResponseEntity<Postagens> getById(@PathVariable Long id) { // variável de caminho
	...
	public ResponseEntity<Postagens> put(@Valid @RequestBody Postagens postagens) {
		return postagemRepository.findById(postagens.getId())
				.map(resposta -> ResponseEntity.status(HttpStatus.OK).body(postagemRepository.save(postagens)))
				.orElse(ResponseEntity.status(HttpStatus.NOT_FOUND).build());
	}
